...
import {
  ...
  TUIVideoQuality,
  TUIVideoStreamType,
  TUIMediaDeviceType } from '@tencentcloud/tuiroom-engine-uniapp-app';
...
export const useRoomStore = defineStore('room', {
  ...
  getters: {
    ...
    localStream: (state) => {
      const { userId, userName, avatarUrl, hasAudioStream, hasVideoStream, userRole } = state.localUser;
      Object.assign(
        state.localUser.cameraStreamInfo,
        { userId, userName, avatarUrl, hasAudioStream, hasVideoStream, userRole },
      );
      return state.localUser.cameraStreamInfo;
    },
    localScreenStream: (state) => {
      const { userId, userName, avatarUrl, hasScreenStream, userRole } = state.localUser;
      Object.assign(
        state.localUser.screenStreamInfo,
        { userId, userName, avatarUrl, hasScreenStream, userRole },
      );
    ...
    remoteStreamObj: (state) => {
      ...
      [...Object.values(state.remoteUserObj)].forEach((userInfo) => {
        ...
        if (onSeat) {
          obj[`${userId}_${TUIVideoStreamType.kCameraStream}`] = Object.assign(userInfo.cameraStreamInfo, { userId, avatarUrl, userName, hasAudioStream, hasVideoStream, userRole, streamType: TUIVideoStreamType.kCameraStream, isVisible: isVideoVisible });
        }
        if (hasScreenStream) {
          obj[`${userId}_${TUIVideoStreamType.kScreenStream}`] = Object.assign(userInfo.screenStreamInfo, { userId, avatarUrl, userName, hasScreenStream, userRole, streamType: TUIVideoStreamType.kScreenStream, isVisible: isScreenVisible });
        }
      });
      return obj;
    },
