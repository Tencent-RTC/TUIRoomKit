...
export class RoomActionManager {
  ...
  public async createRoom(options: {
    ...
    try {
      ...
      const roomParams = {
        ...
        seatMode: roomMode === 'SpeakAfterTakingSeat' ? TUISeatMode.kApplyToTake : undefined,
      };
  ...
  public async enterRoom(options: { roomId: string; roomParam?: RoomParam }) {
    try {
      ...
      if (roomInfo.isSeatEnabled) {
        ...
        this.service.roomStore.isMaster && (await this.service.roomEngine.instance?.takeSeat({ seatIndex: -1, timeout: 0 }));
      }
  ...
  private async doEnterRoom(options: { roomId: string; roomType: TUIRoomType; }) {
    ...
    const roomInfo = await roomEngine.instance?.fetchRoomInfo() as TUIRoomInfo;
