...
class CenterSegmentView: UIView {
    ...
    func initData() {
        ...
        for (index,_)in controllers.enumerated() {
            ...
            btn.frame = CGRect(x: 50 + CGFloat(index) * (width + 30), y: 0, width: width, height: self.segmentScrollVHeight)
        ...
        //分割线
        let downFrame = CGRect(x: 0, y: self.segmentScrollVHeight - 0.5, width: frame.size.width, height: 0.5)
        ...
        self.line.layer.cornerRadius = lineHeight * 0.5
        //初始化位置
