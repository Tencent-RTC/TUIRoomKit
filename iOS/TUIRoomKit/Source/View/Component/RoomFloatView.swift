...
class RoomFloatView: UIView {
    private var isDraging: Bool = false
    private let viewModel: RoomFloatViewModel
    ...
    class func dismiss() {
        DispatchQueue.main.async {
            guard let currentWindow = RoomRouter.getCurrentWindow() else { return }
            for view in currentWindow.subviews where view is RoomFloatView {
