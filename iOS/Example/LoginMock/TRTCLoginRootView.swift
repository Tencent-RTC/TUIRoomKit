...
class TRTCLoginRootView: UIView {
    ...
    lazy var loginBtn: UIButton = {
        ...
        btn.setBackgroundImage(UIColor(0x006EFF).trans2Image(), for: .normal)
        btn.titleLabel?.font = UIFont(name: "PingFangSC-Medium", size: 18)
        btn.layer.shadowColor = UIColor(0x006EFF).cgColor
    ...
    private func createSpacingLine() -> UIView {
        let view = UIView(frame: .zero)
        view.backgroundColor = UIColor(0xEEEEEE)
        return view
    }
    ...
    private func createTextField(_ placeholder: String) -> UITextField {
        ...
        textField.textColor = UIColor(0x333333)
    ...
    @objc func keyboardFrameChange(noti: Notification) {
        ...
        guard let value = info[UIResponder.keyboardFrameEndUserInfoKey],
              value is CGRect else {
            return
        }
