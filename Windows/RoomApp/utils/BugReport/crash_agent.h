...
enum class CrashAgentResult : uint32_t { kSuccess = 0, kNotFound };
...
class ICrashAgent {
 public:
  virtual void Install(const wchar_t* product_name = nullptr,
  ...
  virtual void SetBugReportUin(DWORD uin, BOOL succ_login = FALSE) = 0;
...
CA_INLINE CrashAgentResult CreateCrashAgent(const char* agent_name,
                                            ICrashAgent** agent) {
  ...
  auto create_func = reinterpret_cast<PfCreateCrashAgent>(
      ::GetProcAddress(handle, "CreateCrashAgent"));
