...
function handleTouchMove(e:any) {
  ...
  if (Math.abs(X) > Math.abs(Y) && X > 0) {
    ...
    if (enlargeStream.value) {
      currentStatus.value.status = 'grid';
      currentStatus.value.showSmallStream === null;
      handleSixEqualPointsLayout();
    }
    if (currentPageIndex.value === (showPageDots.value - 1)) return;
    // 向左滑动
    handleTurnPageRight();
  }
}
...
function handleLargestVoice(userVolumeList: any) {
  ...
  if (currentStatus.value?.showSmallStream !== null && currentStatus.value?.showSmallStream !== 'undefined') {
    if (currentStatus.value?.showSmallStream.audioVolume !== 0) return;
  }
