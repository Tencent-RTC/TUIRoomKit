<template>
  ...
  >
    <div
      :id="playRegionDomId"
      :class="[showVoiceBorder ? 'stream-region-mobile-border' : 'stream-region-mobile']"
    >
    </div>
    <div
      v-if="!stream.hasVideoStream && !stream.hasScreenStream"
      ref="centerUserInfoRef"
      :class="[showVoiceBorder ?
        'center-user-info-container-border' : 'center-user-info-container']"
    >
    ...
    <div v-if="showEnlargeIcon && showRoomTool" class="icon-box" @click="handleChooseStream(stream)">
      <span class="enlarge-mobile-down"></span>
      <span class="enlarge-mobile-up"></span>
    </div>
...
import { TUIVideoStreamType, TRTCVideoStreamType, TRTCVideoFillMode, TRTCVideoMirrorType, TRTCVideoRotation } from '@tencentcloud/tuiroom-engine-wx';
...
interface Props {
  ...
  stream: StreamInfo,
  enlargeDomId?: string,
  currentChooseStream: StreamInfo,
}

const props = defineProps<Props>();

const emit = defineEmits(['dblclick', 'choose-click']);

const streamRegionRef = ref();
const centerUserInfoRef = ref();
const showVoiceBorder = computed(() => props.status === 'grid' && props.stream.audioVolume !== 0);
const showEnlargeIcon = computed(() => props.status === 'grid' && props.currentChooseStream === props.stream);
...
function handleChooseStream(stream:StreamInfo) {
  emit('choose-click', stream);
}
watch(
...
.user-stream-container {
  ...
  .center-user-info-container-border{
    position: absolute;
    top: 0;
    left: 0;
    width: 100%;
