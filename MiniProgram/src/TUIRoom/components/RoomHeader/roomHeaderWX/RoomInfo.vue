<template>
  <div class="conference-container">
    <div v-if="isShowRoomInfoTitle" class="title-container" @click="toggleShowRoomInfoStatus">
      <span>{{ masterUserName }}</span>
      <span class="text">{{ t('video conferencing') }}</span>
    ...
    <div v-if="isShowRoomInfo" class="roomInfo-container">
      <div ref="roomInfoRef" class="roomInfo-container-main">
        <div class="roomInfo-title">
          <span>{{ masterUserName }}</span>
          <span class="master-header">{{ t('video conferencing') }}</span>
          <span v-if="isWeChat" class="cancel" @click="handleHiddenRoomInfo">{{ t('Cancel') }}</span>
          <div>
          </div>
        </div>
...
const masterUserName = computed(() => (roomStore.getUserName(masterUserId.value)) || masterUserId);
...
async function  onCopy(value: string | number) {
  ...
  ElMessage({
    message: code === 0 ? t('Copied successfully') : t('Copied failure'),
