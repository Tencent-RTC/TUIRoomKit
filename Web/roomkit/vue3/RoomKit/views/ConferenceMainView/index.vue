...
watch(() => [currentRoom.value?.roomType, currentRoom.value?.roomOwner.userId, loginUserInfo.value?.userId], ([newRoomType, newRoomOwnerUserId, loginUserId]) => {
  if (newRoomType === RoomType.Webinar) {
    if (newRoomOwnerUserId === loginUserId) {
      participantViewLayout.value = RoomLayoutTemplate.FloatMixLayout;
    } else {
      participantViewLayout.value = RoomLayoutTemplate.LiveAudienceLayout;
    }
  }
}, { immediate: true });
