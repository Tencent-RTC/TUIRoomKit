...
export class ErrorHandler {
  ...
  public handleError(error: any, functionName: string) {
  ...
  private handleOnError(error: any) {
    ...
    if (error.code && String(error.code).indexOf('-11') === 0) {
      if (!this.mediaDeviceErrorHandler) {
        this.mediaDeviceErrorHandler = new MediaDeviceErrorHandler(this.service);
      }
...
class MediaDeviceErrorHandler {
  ...
  private getMediaDeviceErrorTitle(deviceType: MediaDeviceType) {
    return this.service.t('Unable to use the device', { deviceType: this.service.t(deviceType) });
  }

  private getMediaDeviceErrorMsg(deviceType: MediaDeviceType) {
    return this.service.t('media device failed to open, please check the media device and try again', { deviceType: this.service.t(deviceType) });
  }

  private getMediaDeviceNotAuthorizedMsg(deviceType: MediaDeviceType) {
    return this.service.t('The current device is not authorized, please allow access to the device permissions', { deviceType: this.service.t(deviceType) });
  }

  private getMediaDeviceOccupiedMsg(deviceType: MediaDeviceType) {
    return this.service.t('The current device is occupied by other apps, try to close other apps or change the device', { deviceType: this.service.t(deviceType) });
  }

  private getTurnOnDevicePrivilegesTitle(deviceType: MediaDeviceType) {
    return this.service.t('Turn on device privileges', { deviceType: this.service.t(deviceType) });
  }

  private getTurnOnDevicePrivilegesMsg(deviceType: MediaDeviceType) {
    return this.service.t('You can go to "System Preferences - Security & Privacy - Device" to enable device permissions', { deviceType: this.service.t(deviceType) });
  }
  ...
  private goToPermissionSettings(deviceType: MediaDeviceType) {
    ...
    shell.openExternal(`x-apple.systempreferences:com.apple.preference.security?${privacyTypeObj[deviceType]}`);
  }
}
