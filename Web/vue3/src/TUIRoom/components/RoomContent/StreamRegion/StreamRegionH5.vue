...
const emit = defineEmits(['click', 'choose-click']);

const streamRegionRef = ref();
const centerUserInfoRef = ref();
const showVoiceBorder = computed(() => props.status === 'grid' && props.stream?.audioVolume !== 0 && props.stream?.streamType !== TUIVideoStreamType.kScreenStream);
