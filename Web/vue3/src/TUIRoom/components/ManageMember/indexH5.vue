<template>
  <div class="manage-member-container">
    ...
    <div v-if="showManageAllUserDialog" class="popup-dialog-audio">
      <div class="popup-dialog-audio-title">{{ dialogTitleInfo }}</div>
      <div class="popup-button">
        <button class="popup-button-cancel" @touchstart="showManageAllUserDialog = false">{{ t('Cancel') }}</button>
        <button class="popup-button-mute" @touchstart="doToggleManageMember">{{ dialogActionInfo }}</button>
      </div>
    </div>
  </div>
</template>
  ...
  .manage-member-container {
    ...
    .popup-dialog-audio,.popup-dialog-video {
      ...
      .popup-button{
        display: flex;
        margin-top: 6vh;
        .popup-button-cancel,.popup-button-mute{
          margin-right: 0;
          width: 32vw;
          height: 5vh;
          background: var(--popup-mute-background-color-h5);
