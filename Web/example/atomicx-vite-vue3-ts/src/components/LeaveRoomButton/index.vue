...
import { TUIButton, TUIDialog, TUISelect, TUIOption, TUIToast, useUIKit } from '@tencentcloud/uikit-base-component-vue3';
import { RoomParticipantRole, useRoomParticipantState, useRoomState } from 'tuikit-atomicx-vue3/room';
import { useRouter } from 'vue-router';

const { t } = useUIKit();
const router = useRouter();

const { currentRoom, leaveRoom, endRoom } = useRoomState();
const { localParticipant, participantList, transferOwner } = useRoomParticipantState();
...
const autoTransferAndLeave = async () => {
  ...
  try {
    ...
    TUIToast.success({
      message: t('Room.TransferSuccessAndLeave', {
        name: targetParticipant.userName || targetParticipant.userId,
      }),
    });

    router.replace('/home');
  } catch (_error) {
    TUIToast.error({ message: t('Room.TransferFailed') });
  }
};
...
const handleTransferAndLeave = async () => {
  if (!selectedUserId.value || !currentRoom.value?.roomId || isTransferring.value) {
    return;
  }
  ...
  try {
    ...
    const selectedParticipant = otherParticipants.value.find(p => p.userId === selectedUserId.value);
    TUIToast.success({
      message: t('Room.TransferSuccessAndLeave', {
        name: selectedParticipant?.userName || selectedUserId.value,
      }),
    });
