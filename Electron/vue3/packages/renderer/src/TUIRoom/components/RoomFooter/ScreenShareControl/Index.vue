<template>
  <div class="screen-share-control-container">
    ...
    <Dialog :model-value="showStopShareRegion" width="420px" :title="t('End sharing')" :modal="true">
      ...
      <template #footer>
        <span>
          <Button class="dialog-button" size="default" @click="stopScreenShare">{{ t('End sharing') }}</Button>
    ...
    >
      ...
      <template #footer>
        <Button size="default" @click="onPermissionScreenShare">
          {{ t('Open the system preferences settings') }}
        </Button>
        <Button class="button" type="primary" size="default" @click="showPermissionVisible = false">
          {{ t('Cancel') }}
        </Button>
      </template>
    </Dialog>
