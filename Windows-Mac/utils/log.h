...
public:
    ...
    ~Log();
public:
    static void SetLogPath(const std::string& path, const std::string& app_name) { log_file_path_ = path; app_name_ = app_name; };
...
#if defined(USE_LOG)
    #define SET_LOG_PATH(path, app_name)       Log::SetLogPath(path, app_name)
