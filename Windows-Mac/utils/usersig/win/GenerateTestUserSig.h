 ...
 *
 *            The correct method is to deploy the `UserSig` calculation code and encryption key on your server so that your application can request a `UserSig` from your server, which will calculate it whenever one is needed.
...
public:
    ...
    /*
     *  Access: Tencent Cloud web console -> Realtime audio and video -> your application (EG customer service call)-> Account information panel appid/bizid
     */
     ...
     * Once your key is disclosed, attackers will be able to steal your Tencent Cloud traffic.
     *
     * The correct method is to deploy the `UserSig` calculation code and encryption key on your server so that
     * your application can request a `UserSig` from your server, which will calculate it whenever one is needed.
     * Given that it is more difficult to hack a server than a client application, server-end calculation can better protect your key.
     *
     * 文档：https://cloud.tencent.com/document/product/647/17275#Server
     */
